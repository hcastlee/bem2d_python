...
class pybem2d:
  ...
  def get_coef_slp(I,J):
      ...
      # determin whether integration is singular
      
      # compute coordinates in real space
      
      # add up components from each basis function attached to global dof
  ...
  def singular_quad_rule_p1(self):
      """creates a singular quadrature rule for pair of elements.
     don't need to split the integration up, since the singular integration
     takes place over two elements. Need to reverse the integration for the first element."""
  ...
  def define_shape_functions(self):
      ...
      if self.basisType == 'discontinuous':
          ...
          self.basisNum = 1
          self.phin = _np.ones(self.eta, dtype=float64)
          self.phis = _np.ones(self.eta_singular, dtype=float64)
      elif self.basisType == 'continuous':
          self.nBasis = 2;
          self.phin = _np.zeros((2 , self.eta.size) , dtype=float64);
          #regullar shape functions
          # these must go to zero when eta is min, and 1 when eta is max, and vice versa.
          # tthey should sum to one at all points
          self.phin(1,:) = self.eta;
          self.phin(2,:) = 1.0 - self.eta;
          # singuar shape functions times the jacobian
          self.phis = _np.zeros((2,self.eta_singular.size), dtype=float64)
          self.phis(1,:) = self.eta_singular;
          self.phis(2,:) = 1.0 - self.eta_singular;
  ...
  def prepare_quad_rules(self):
      """choose appropriate quadrature rule and stores in
      self.eta_singular and self.wts_singular. Each of these arrays
      has size nBasisFunctions x nQuad"""
    self.create_quad_rule()
    self.create_refined_composite_quad_rule()
    if self.basisType == 'discontinuous':
        self.singular_quad_rule_dp0()
        self.eta_singular = self.eta_singular_dp0;
        self.wts_singular = self.wts_singular_dp0;
    elif  self.basisType == 'continuous':
        self.singular_quad_rule_p1()
        self.eta_singular = self.eta_singular_p1;
        self.wts_singular = self.wts_singular_p1;
